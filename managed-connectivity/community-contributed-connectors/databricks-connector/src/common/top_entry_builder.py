...
def _create_entry(config: Dict[str, str], entry_type: EntryType, catalog_name: str = ""):
    """Creates a Dataplex Entry."""
    entry = dataplex_v1.Entry()
    entry.name = nb.create_name(config, entry_type, catalog_name)
    
    entry.entry_type = entry_type.value.format(
        project=config["target_project_id"], location=config["target_location_id"])
    
    entry.fully_qualified_name = nb.create_fqn(config, entry_type, catalog_name)
    entry.parent_entry = nb.create_parent_name(config, entry_type, catalog_name)
...
def create(config, entry_type: EntryType, catalog_name: str = ""):
    """Creates a dataplex entry, packs it to Import Item and converts to json."""
    import_item = _entry_to_import_item(_create_entry(config, entry_type, catalog_name))
