...
def create_fqn(config: Dict[str, str], entry_type: EntryType,
    ...
    if entry_type == EntryType.CATALOG:
        ...
        return f"{SOURCE_TYPE}:`{config['server_hostname']}`"
    if entry_type == EntryType.DB_SCHEMA:
        instance = create_fqn(config, EntryType.CATALOG)
        return f"{instance}.{schema_name}"
    if entry_type in [EntryType.TABLE, EntryType.VIEW]:
        instance = create_fqn(config, EntryType.CATALOG)
        return f"{instance}.{schema_name}.{table_name}"
    return ""
...
def create_name(config: Dict[str, str], entry_type: EntryType,
    ...
    if entry_type == EntryType.CATALOG:
        ...
        return name_prefix + config["server_hostname"].replace(".", "@")
    if entry_type == EntryType.DB_SCHEMA:
        instance = create_name(config, EntryType.CATALOG)
        return f"{instance}/database_schemas/{schema_name}"
